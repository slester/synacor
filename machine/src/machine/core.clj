...
(defn read-file [file-path] (IOUtils/toByteArray (io/input-stream file-path)))
...
(defn resolve-val [rs bs v] (println "resolving" v)
  ...
  (if (> first-register v)
    ...
    (if (> first-register (bs v))
      (bs v)
      ;; value is in a register, so it needs resolving
      (resolve-val rs bs (bs v)))
    ;; value is in a register, so get its value
...
(defn execute [pos mem rs s]
  ...
  (case (mem pos)
    ...
    2 (let [a (mem (inc pos))
            v (val-of rs a)]
        (println "PUSH" a "onto the stack")
        [(+ 2 pos) mem rs (conj s v)])
    ;; pop: 3 a - remove the top element from the stack and write it into <a>; empty stack = error
    3 (let [a (mem (inc pos))
            r (rkey a)
            v (peek s)]
        (println "POP and write to" a)
    ...
    16 (let [a (mem (inc pos))
             b (mem (+ 2 pos))
             a-val (rval rs a)]
         (println "WMEM value from" b "into memory at address" a)
         (println b "->" a-val)
         [(+ 3 pos)
          (if (>= a-val first-register) mem (assoc mem (int a-val) b))
          (if (>= a-val first-register) (assoc rs (rkey a-val) b) rs)
          s])
